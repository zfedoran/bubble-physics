...
namespace bubblephysics.graphics
{
    ...
    public class LineBatch
    {
        ...
        {
            this.device = device;
            this.vertices = new LineVertex[512];
            this.curr_vertex = 0;
            this.hasBegun = false;
        }
        ...
        public void Add(LineVertex vertex)
        {
            ...
            {
                LineVertex[] tmp = new LineVertex[this.vertices.Length + 512];
        ...
        public void End()
        {
            if (!this.hasBegun)
                throw new Exception("You must call Begin() first");
            if (!(this.curr_vertex % 2 == 0))
                throw new Exception("You have not added enough vertices");

            this.hasBegun = false;

            device.DrawUserPrimitives<LineVertex>(PrimitiveType.LineList, this.vertices, 0, (this.curr_vertex+1) / 2);
        }
    }
}
